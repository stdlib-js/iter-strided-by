...
function iterStridedBy( iterator, fcn, offset, eager, thisArg ) {
	...
	if ( !isIteratorLike( iterator ) ) {
		throw new TypeError( format( '0Pv46', iterator ) );
	}
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '0Pv2H', fcn ) );
	}
	if ( arguments.length === 3 ) {
		...
		if ( !isNonNegativeInteger( offset ) ) {
			throw new TypeError( format( '0Pv2m', offset ) );
		}
		idx = offset;
		if ( isBoolean( eager ) ) {
			bool = eager;
		...
		idx = offset;
		if ( !isBoolean( eager ) ) {
			throw new TypeError( format( '0Pv4J', eager ) );
		}
		bool = eager;
		ctx = thisArg;
	} else {
	...
	*/
	function next() {
		...
		if ( !isPositiveInteger( stride ) ) {
			throw new TypeError( format( '0Pv4K', stride ) );
		}
		idx += stride;
		return v;
	}
